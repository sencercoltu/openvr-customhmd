...
namespace UdpClientServer
{
	...
	*
	* \exception UdpClientServer_runtime_error
	...
	*/
	CUdpClient::CUdpClient(const std::string& addr, int port)
		: f_port(port)
		, f_addr(addr)
	{
		...
		{
			throw UdpClientServer_runtime_error(("invalid address or port: \"" + addr + ":" + decimal_port + "\"").c_str());
		}
		f_socket = socket(f_addrinfo->ai_family, SOCK_DGRAM , IPPROTO_UDP);
		if (f_socket == INVALID_SOCKET)
		{
			freeaddrinfo(f_addrinfo);
			throw UdpClientServer_runtime_error(("could not create socket for: \"" + addr + ":" + decimal_port + "\"").c_str());
		}
		int opt = 1;
		if (setsockopt(f_socket, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt, sizeof(int)) < 0)
		{
			freeaddrinfo(f_addrinfo);
			closesocket(f_socket);
			f_socket = INVALID_SOCKET;
			throw UdpClientServer_runtime_error(("could not bind UDP socket for: \"" + addr + ":" + decimal_port + "\"").c_str());
		}
	...
	*/
	SOCKET CUdpClient::get_socket() const
	{
		return f_socket;
	}
	...
	*/
	std::string CUdpClient::get_addr() const
	{
		return f_addr;
	}
	...
	*/
	int CUdpClient::send(const char *msg, int size)
	{
		return sendto(f_socket, msg, size, 0, f_addrinfo->ai_addr, (int) f_addrinfo->ai_addrlen);
	}
	...
	*
	* \exception UdpClientServer_runtime_error
	* The UdpClientServer_runtime_error exception is raised when the address
	...
	*/
	CUdpServer::CUdpServer(const std::string& addr, int port)
		: f_port(port)
		, f_addr(addr)
	{
		char decimal_port[16];
		...
		if (r != 0 || f_addrinfo == NULL)
		{
			throw UdpClientServer_runtime_error(("invalid address or port for UDP socket: \"" + addr + ":" + decimal_port + "\"").c_str());
		}
		f_socket = socket(f_addrinfo->ai_family, SOCK_DGRAM, IPPROTO_UDP);
		if (f_socket == INVALID_SOCKET)
		{
			freeaddrinfo(f_addrinfo);
			throw UdpClientServer_runtime_error(("could not create UDP socket for: \"" + addr + ":" + decimal_port + "\"").c_str());
		}
		int opt = 1;
		if (setsockopt(f_socket, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt, sizeof(int)) < 0)
		{
		...
		r = bind(f_socket, f_addrinfo->ai_addr, (int)f_addrinfo->ai_addrlen);
		if (r != 0)
		{
			freeaddrinfo(f_addrinfo);
			closesocket(f_socket);
			throw UdpClientServer_runtime_error(("could not bind UDP socket with: \"" + addr + ":" + decimal_port + "\"").c_str());
		}
	}
	...
	*/
	SOCKET CUdpServer::get_socket() const
	{
		return f_socket;
	}
	...
	*/
	std::string CUdpServer::get_addr() const
	{
		return f_addr;
	}
	...
	*/
	int CUdpServer::recv(char *msg, int max_size)
	{
	...
	*/
	int CUdpServer::timed_recv(char *msg, int max_size, int max_wait_ms)
	{
		fd_set r,w,e;
		FD_ZERO(&r);
		FD_ZERO(&w);
		...
		int retval = select((int)f_socket + 1, &r, &w, &e, &timeout);
		if (retval == -1)
		{
			// select() set errno accordingly
			return -1;
		}
		if (retval > 0 && FD_ISSET(f_socket, &r))
		{
			// our socket has data
			return ::recv(f_socket, msg, max_size, 0);
		}
